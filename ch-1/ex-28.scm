...
; the numbers a<n, computing a^(n-1 )in this way will reveal a nontrivial square
...
(define (expmod base exp m)
  (define (even-branch x n)
    (if (or (= x 1) (= x (- n 1)) (not (= (remainder (square x) n) 1)))
        (remainder (square x) n)
        0))
  (cond ((= exp 0) 1)
        ((even? exp)
         (even-branch (expmod base (/ exp 2) m)
                    m))
        (else
         (remainder (* base (expmod base (- exp 1) m))
                    m))))

(define (complete-miller-rabin n)
  (define (try-it a)
    (= (expmod a n n) a))
...
(complete-miller-rabin 13) ; #t
(complete-miller-rabin 83) ; #t
(complete-miller-rabin 20) ; #f
(complete-fermat-test 6601) ; #f
